<template>
	<div class="card border-secondary  mb-3" >
	  <div class="card-header h4">Size Add</div>
	  <div class="card-body ">
	    	...
	    	<div class="row">
	    		...
	    		<div class="col-6">
					  <div class="form-group mt-2">
					    <label for="title">Size Label</label>
					    <input type="text" class="form-control mt-1" v-model="sizeLabel">
					    <!-- <small id="emailHelp" class="form-text text-muted">We'll never share your email with anyone else.</small> -->
					  </div>
	    		</div>
	    		<div class="col-12 mt-2 d-flex flex-row-reverse">
	    			
	    			<button class="btn btn-info m-2 text-light" @click="addSize()">
	    				<icon name="save" size="20px"/> Add</button>
	    			<button class="btn btn-warning m-2 text-light" @click="resetSizeForm()"><icon name="switch" size="20px"/>Reset</button>
	    		</div>
	    	</div>
		</form>
	  </div>
	...
	methods:{
		...
		addSize(){
			if(this.sizeFormValidation()){
				...
				this.$http.post("size/add",formData,config)
				 .then(response=>{
