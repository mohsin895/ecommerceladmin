<template>
    <div class="product-list">
        ...
        <div class="add-customer">
            <router-link :to="{name:''}"  class="add-btn">Add New Customer</router-link>
        </div>
        <br>
            ...
            <table class="table table-bordered">
                 ...
                 <slot v-for="(dataInfo,index) in dataList.data">
                    <tr>
                      ...
                      <td>
                      	<slot v-if="dataInfo.avatar!=null">
	                      	<img v-lazy="dataInfo.avatar" :alt="dataInfo.name" style="height:70px;width:70px;border-radius:50%;border:1px solid green;">
	                    </slot>
                      </td>
                      <td>{{dataInfo.name}}</td>
                      <td>{{dataInfo.email}}</td>
                      <td>{{dataInfo.phone}}</td>
                      <td>{{dataInfo.dob | formatDate}}</td>
                      
                      <td>
                      	<div class="form-check form-switch">
						  <input class="form-check-input" type="checkbox" role="switch" :id="'is_verify_'+dataInfo.id" :checked="(dataInfo.is_verify==1) ? 'true':false" @click="changeDataVerifyStatus(dataInfo.id,dataInfo.is_verify)" :disabled="(dataInfo.is_verify==1) ? 'true':false">
						  <label class="form-check-label badge" :for="'is_verify_'+dataInfo.id" :class="(dataInfo.is_verify==1) ? 'bg-success':'bg-warning'"> {{dataInfo.is_verify | yesNo}}</label>
						</div>
                      </td>
                      <td>
                      	<div class="form-check form-switch">
                      ...
                      <td>
          			           <router-link :to="{name:'CustomerEdit',params:{dataId:dataInfo.id}}" class="btn btn-warning btn-sm text-light">
...
export default{
    ...
    methods:{
        ...
        async changeDataVerifyStatus(dataId,status){
            ...
            await this.$http.post('/customer/verify',formData,config)
                 .then(response=>{
        ...
        async changeDataStatus(dataId,status){
            ...
            await this.$http.post('/customer/change/status',formData,config)
                 .then(response=>{
                    this.errMsgFlag=response.data.errMsgFlag;
                    this.msgFlag=response.data.msgFlag;
                    this.msg=response.data.msg;
