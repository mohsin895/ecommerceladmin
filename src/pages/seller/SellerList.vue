<template>
    <div class="product-list">
        ...
        <div class="add-seller">
            <router-link :to="{name:'SellerAdd'}" class="add-btn">Add New Seller</router-link>
        </div>
        <br>
        <div class="datatables">
            <table class="table table-bordered">
              ...
              <label class="form-check-label badge" :for="'is_verify_'+dataInfo.id" :class="(dataInfo.is_verify==1) ? 'bg-success':'bg-warning'"> {{dataInfo.is_verify | yesNo}}</label>
                      ...
                      <td>
          			           <router-link :to="{name:'SellerEdit',params:{dataId:dataInfo.id}}" class="btn btn-warning btn-sm text-light">
...
export default{
    ...
    methods:{
        ...
        async changeDataVerifyStatus(dataId,status){
            ...
            await this.$http.post('/seller/verify',formData,config)
                 .then(response=>{
        ...
        async changeDataStatus(dataId,status){
            ...
            await this.$http.post('/seller/change/status',formData,config)
                 .then(response=>{
                    this.errMsgFlag=response.data.errMsgFlag;
                    this.msgFlag=response.data.msgFlag;
                    this.msg=response.data.msg;
