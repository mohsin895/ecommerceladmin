<template>
    <div class="product-list">
        ...
        <div class="add-category">
            <router-link :to="{name:'CategoryAdd'}" class="add-btn">Add New Category</router-link>
        </div>
        <br>
            ...
            <table class="table table-bordered">
                 ...
                 <slot v-for="(dataInfo,index) in dataList.data">
                    <tr>
                      ...
                      <td>
                      	<slot v-if="dataInfo.category_image!=null">
	                      	<img v-lazy="dataInfo.category_image.base_url+dataInfo.category_image.category_image" :alt="dataInfo.name" height="70" width="70">
	                    </slot>
                      </td>
                      <td>{{dataInfo.title}}</td>
                      <td>
                      	<slot v-if="dataInfo.parent_info!=null">
                      		{{dataInfo.parent_info.title}}
                      	</slot>
                      </td>
                      <td>{{dataInfo.look_type | filterLookType}}</td>
                      <td>
                      ...
                      <td>
        			           <router-link :to="{name:'CategoryEdit',params:{dataId:dataInfo.id}}" class="btn btn-warning btn-sm text-light">
...
export default{
    ...
    methods:{
        ...
        async changeDataStatus(dataId,status){
            ...
            await this.$http.post('/category/change/status',formData,config)
                 .then(response=>{
