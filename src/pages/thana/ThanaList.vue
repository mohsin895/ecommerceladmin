<template>
    <div class="product-list">
        ...
        <br>
        <div class="add-thana">
            <router-link :to="{name:'ThanaAdd'}" class="add-btn">Add New Thana</router-link>
        </div>
        <br>
                    ...
                    <tr>
                      ...
                      <td>
                        <slot v-if="dataInfo.district_info!=null">
                          {{dataInfo.district_info.name}}
                        </slot>
                      </td>
                      <td>
                        <slot v-if="dataInfo.district_info!=null">
                          <slot v-if="dataInfo.district_info.division_info!=null">
                            {{dataInfo.district_info.division_info.name}}
                          </slot>
                        </slot>
                      </td>
                      <td>
                        <span class="badge" :class="(dataInfo.inter_city==1) ? 'bg-success':'bg-danger'">{{dataInfo.inter_city | yesNo}} </span>
                      </td>
                      <td>
                      ...
                      <td>
                           <router-link :to="{name:'ThanaEdit',params:{dataId:dataInfo.id}}" class="btn btn-warning btn-sm text-light">
...
export default{
    ...
    methods:{
        ...
        async changeDataStatus(dataId,status){
            ...
            await this.$http.post('/thana/change/status',formData,config)
                 .then(response=>{
