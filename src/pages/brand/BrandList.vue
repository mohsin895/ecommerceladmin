<template>
    <div class="product-list">
        ...
        <br>
        <div class="add-brand">
            <router-link :to="{name:'BrandAdd'}" class="add-btn">Add New Brand</router-link>
        </div>
        <br>
            ...
            <table class="table table-bordered">
                 ...
                 <slot v-for="(dataInfo,index) in dataList.data">
                    <tr>
                      ...
                      <td>
                      	<slot v-if="dataInfo.logo!=null">
	                      	<img v-lazy="dataInfo.logo" :alt="dataInfo.name" style="height:70px; width:70px; border:1px solid green; border-radius:50%;">
	                    </slot>
                      </td>
                      <td>{{dataInfo.name}}</td>
                      ...
                      <td>
          			           <router-link :to="{name:'BrandEdit',params:{dataId:dataInfo.id}}" class="btn btn-warning btn-sm text-light">
...
export default{
    ...
    methods:{
        ...
        async changeDataStatus(dataId,status){
            ...
            await this.$http.post('/brand/change/status',formData,config)
                 .then(response=>{
