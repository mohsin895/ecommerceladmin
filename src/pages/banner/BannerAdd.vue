	    ...
	    <form  @submit.prevent="formSubmitPrevent()">
	    	<div class="row">
	    		...
	    		<div class="col-12 mt-2 d-flex flex-row-reverse">
	    			...
	    			<button class="btn btn-info m-2 text-light" @click="addBanner()">
...
export default{
	...
	methods:{
    	...
    	async addBanner()
    	{
    		if(this.formValidation())
    		{
    			this.errMsgFlag=false;
			        ...
			        await this.$http.post("banner/add",formData,config)
			         		.then(response=>{
    	...
    	formValidation()
    	{
    		if(this.$refs.banner.files[0]!=null && this.title!=null && this.description!=null && this.slug!=null && this.category!=null )
    		{
    			return true;
    		}
    		else
    		{
    			if(this.$refs.banner.files[0]==null )
    			{
    				this.errMsg="Please Choose An Image First.";
    				this.$toaster.warning(this.errMsg);
    			}
    			if(this.title==null)
    			{
